...
class ProjectMemberController {
    addProjectMember = async (req, res) => {
        try {
            ...
            const result = await projectMemberService.addProjectMember(project_id, user_id);
            res.status(201).json(result);
        } catch (error) {
            res.status(400).json({ message: 'Failed to add project member', error: error.message });
        }
    }
    ...
    removeProjectMember = async (req, res) => {
        try {
            const { project_id, user_id } = req.body;
            const result = await projectMemberService.removeMember(project_id, user_id);
            res.status(200).json(result);
        } catch (error) {
            res.status(400).json({ message: 'Failed to remove project member', error: error.message });
        }
    }
